...
const [topProducts, setTopProducts] = useState([]);
  ...
  return (
    <div className="bg-gradient-to-br from-sky-50 to-indigo-50 min-h-screen px-6 py-8">
      <h2 className="text-2xl font-semibold text-gray-800 mb-6 text-center">Manage All Products</h2>
...
{!loading && products.length === 0 && (
  <div className="flex items-center justify-center min-h-[60vh]">
    <p className="text-4xl font-bold text-gray-500">No products to show</p>
  </div>
)}
...
{topProducts.map((product, index) => (
  ...
  >
    ...
    />
    <p className="text-gray-800 font-semibold text-center">Seller ID: {product.SELLERID}</p>
    <p className="text-gray-700 text-center mt-1">Total Ordered: {product.TOTAL_ORDERS}</p>
    <p className="text-sm text-gray-600 text-center">Price: ৳{product.PRICE}</p>
  </div>
))}
        ...
        {products.map((product) => (
          ...
          >
            <div>
              ...
              <p className="text-sm text-gray-600 mb-2">Seller ID: {product.SELLERID}</p>
            </div>
            <button
